...
var refreshFAQNoticeTable = function (data) {
    var saleProductTable = document.querySelector('#FAQNoticeTable');
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        a.href = 'FAQDetail_ADMIN.html?' + 'No=' + data[i].No;
...
var refreshFAQTable = function (data) {
    var saleProductTable = document.querySelector('#FAQTable');
    var tbody = saleProductTable.querySelector('tbody');
    tbody.parentNode.removeChild(tbody);
    tbody = document.createElement('tbody');
    saleProductTable.appendChild(tbody);
...
var refreshPageList = function () {
    ...
    var a = document.createElement('a');
    var hrefStr = "FAQ_ADMIN.html?page=" + String(((pageN - 1) <= 0) ? 1 : (pageN - 1));
    if (params.length > 1)
    ...
    for (var i = 0; i < frontPageN - 1; i++) {
        ...
        var hrefStr = "FAQ_ADMIN.html?page=" + String(tmpPageNum);
        if (params.length > 1)
            hrefStr += "&option=" + params[1] + "&keyword=" + params[2];
        var a = document.createElement('a');
        a.href = hrefStr;
    ...
    var hrefStr = "FAQ_ADMIN.html?page=" + String(pageN);
    if (params.length > 1)
        hrefStr += "&option=" + params[1] + "&keyword=" + params[2];
    var a = document.createElement('a');
    a.href = hrefStr;
    ...
    var a = document.createElement('a');
    var hrefStr = "FAQ_ADMIN.html?page=" + String((pageN * SHOW_NUM > count) ? pageN : (pageN + 1));
    if (params.length > 1)
        hrefStr += "&option=" + params[1] + "&keyword=" + params[2];
    a.href = hrefStr;
    a.className = "list-group-item list-group-item-primary pageListItem"
...
var getFAQNotice = function () {
    $.ajax({
        type: "POST",
        url: '/getBoard/FAQ/notice',
        success: function (data) {
            refreshFAQNoticeTable(data);
        },
        error: function (e) {
...
var searchFunc = function () {
    ...
    var hrefStr = 'FAQ_ADMIN.html?page=1';
    if (searchKeyword != '')
