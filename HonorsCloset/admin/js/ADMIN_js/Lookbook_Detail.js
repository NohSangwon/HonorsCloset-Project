...
var seasonName = year + ' ' + (season == '1' ? 'Spring' : 'Fall') + ' 컬렉션';
seasonNameH.innerHTML = seasonName;

var imageList = null;
$.ajax({
    type: "POST",
    url: '/Lookbook/getImageList',
    data: {
...
var deleteImg = function (season_, realImg_) {
    var season = season_;
...
function createImages() {
    ...
    for (var i = 0; i < data.realImg.length; i++) {
        if (i % 2 == 0) {
            var row = document.createElement('div');
            row.className = 'row';
            imageArea.appendChild(row);
        ...
        col.className = 'col-lg-6 col-sm-6 img-item';
        row.appendChild(col);
        ...
        img.id = "realImg";
        img.src = '../src/lookbook/' + seasonCode + '/' + data.realImg[i] + '.jpg';
        ...
        deleteBtn.id = data.realImg[i];
        deleteBtn.innerHTML = "X";
        deleteBtn.addEventListener('click', deleteImg(seasonCode, data.realImg[i]));
...
var insertNewImgFunc = function () {
    ...
    $('#imgInsertForm').ajaxForm({
        url: "/Lookbook/insertImg/" + seasonCode + "/" + cnt,
...
var deleteBehindcutFunc = function (season_) {
    if (confirm(year + ' ' + (season == '1' ? 'Spring' : 'Fall') + ' 룩북을 삭제하시겠습니까?')) {
        $.ajax({
            type: "POST",
            url: '/Lookbook/deleteLookbook',
            data: {
                seasonCode: seasonCode,
            },
            success: function (data) {
                alert(year + ' ' + (season == '1' ? 'Spring' : 'Fall') + ' 룩북을 삭제하였습니다');
...
var updateVideoFunc = function (season_) {
    if (confirm('동영상을 업로드하시겠습니까?')) {
        $('#videoInsertForm').ajaxForm({
            url: "/Lookbook/updateVideo/" + seasonCode,
            enctype: "multipart/form-data",
            beforeSubmit: function (data, form, option) {
