...
var goBehindeDetailFunc = function (idx) {
    var index = idx;
    return function () {
        window.location.href = 'MemberCut_Detail?season=' + behindcutList[index].Season;
    }
}
...
var getBehindCutList = function () {
    $.ajax({
        type: "POST",
        url: '/MemberCut/getSeasonList/DESC',
        async: false,
...
var insertBehindeCutSubmitFunc = function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data == 'success') {
                alert(yearText + ' ' + seasonText + ' 멤버컷을 등록했습니다');
