...
var salesHistoryDetaileEventFunc = function (idx) {
    ...
    return function () {
        ...
        var td = document.createElement('td');
        td.id = 'OrderDate';
        var orderDate = salesHistory[currIdx].OrderDate;
        orderDate = orderDate.substring(0, 4) + '/' + orderDate.substring(4, 6) + '/' + orderDate.substring(6, 8) + ' ' + orderDate.substring(8, 10) + ':' + orderDate.substring(10, 12) + ':' + orderDate.substring(12, 14);
        ...
        var customerDetailTable = document.querySelector('#customerDetailTable');
        var customerID = customerDetailTable.querySelector('#customerID');
        var name = customerDetailTable.querySelector('#name');
        var phone = customerDetailTable.querySelector('#phone');
        
        customerID.innerHTML = salesHistory[currIdx].Customer_ID;
        
        if (salesHistory[currIdx].customer == undefined || salesHistory[currIdx].customer == null) {
            name.innerHTML = '-';
            phone.innerHTML = '-';
        } else {
            name.innerHTML = salesHistory[currIdx].customer.Name;
            phone.innerHTML = salesHistory[currIdx].customer.PhoneNum;
        }
...
var refreshSalesHistoryTable = function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        $.ajax({
            type: "POST",
            url: '/Member/getCustomerByID',
            data: {
                id: salesHistory[i].Customer_ID
            },
            async: false,
            success: function (data) {
                salesHistory[i].customer = data[0];
            },
            error: function (e) {
        ...
        var td = document.createElement('td');
        td.id = 'Customer_Name';
        if (salesHistory[i].customer == undefined || salesHistory[i].customer == null) {
            td.innerHTML = '-';
        } else {
            td.innerHTML = salesHistory[i].customer.Name;
        }
        tr.appendChild(td);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (salesHistory.length == 0) {
                    alert(fromDateStr + ' ~ ' + toDateStr + ' : 조회결과가 없습니다.');
                } else {
...
var confirmPayment = function () {
    ...
    if (salesHistory[currIdx].isCanceled == '1') {
        alert('이 주문은 취소된 주문 입니다.');
        return;
    }
    if (!confirm('주문코드 : ' + salesHistory[currIdx].No + ' - 입금확인 처리하시겠습니까?')) return;
...
document.querySelector('#confirmPaymentBtn').addEventListener('click', confirmPayment);
