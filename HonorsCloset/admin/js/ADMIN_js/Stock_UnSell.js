...
// DataTable Load Event
$.ajax({
    type: "POST",
    url: '/getProducts/UnSelled',
...
$("a[data-toggle='tab']").on("show.bs.tab", function (event) {
    ...
    $.ajax({
        type: "POST",
        url: '/getProducts/BySpecification_UnSell',
        data: DATA,
        success: function (data) {
            var DataTable = document.getElementById('DataTable');
            // DataTable.getElementsByTagName('tbody').item(0).remove();
...
function OverThumbnailEvent(event) {
    //    var target_tr = event.target.parentElement.closest('tr').rowIndex;
    //    var target_Code = DataTable.rows[target_tr].cells[1].innerText;
