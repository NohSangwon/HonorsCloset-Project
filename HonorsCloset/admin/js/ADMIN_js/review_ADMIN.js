...
var refreshReviewNoticeTable = function (data) {
    var saleProductTable = document.querySelector('#ReviewNoticeTable');
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        a.href = 'reviewDetail_ADMIN.html?' + "No=" + data[i].No;
        a.innerHTML = data[i].Title + '&nbsp';
        td.appendChild(a);
        // 비밀글 -> Lock 아이콘 추가
        var isHidden = data[i].isHidden;
        if(isHidden == '1'){
            var Lock = document.createElement('img');
            Lock.style.width = '10pt';
            Lock.style.height = '10pt';
            Lock.src='src/icon_Lock.png';
...
var refreshReviewTable = function (data) {
    var saleProductTable = document.querySelector('#ReviewTable');
    var tbody = saleProductTable.querySelector('tbody');
    tbody.parentNode.removeChild(tbody);
    tbody = document.createElement('tbody');
    saleProductTable.appendChild(tbody);
...
var refreshPageList = function () {
    ...
    var a = document.createElement('a');
    var hrefStr = "review_ADMIN.html?page=" + String(((pageN - 1) <= 0) ? 1 : (pageN - 1));
    if (params.length > 1)
    ...
    for (var i = 0; i < frontPageN - 1; i++) {
        ...
        var hrefStr = "review_ADMIN.html?page=" + String(tmpPageNum);
        if (params.length > 1)
            hrefStr += "&option=" + params[1] + "&keyword=" + params[2];
        var a = document.createElement('a');
        a.href = hrefStr;
    ...
    var hrefStr = "review_ADMIN.html?page=" + String(pageN);
    if (params.length > 1)
        hrefStr += "&option=" + params[1] + "&keyword=" + params[2];
    var a = document.createElement('a');
    a.href = hrefStr;
    ...
    var a = document.createElement('a');
    var hrefStr = "review_ADMIN.html?page=" + String((pageN * SHOW_NUM > count) ? pageN : (pageN + 1));
    if (params.length > 1)
        hrefStr += "&option=" + params[1] + "&keyword=" + params[2];
    a.href = hrefStr;
    a.className = "list-group-item list-group-item-primary pageListItem"
...
var getReviewNotice = function () {
    $.ajax({
        type: "POST",
        url: '/getBoard/Review/notice',
        success: function (data) {
            refreshReviewNoticeTable(data);
        },
        error: function (e) {
...
var searchFunc = function () {
    ...
    var hrefStr = 'review_ADMIN.html?page=1';
    if (searchKeyword != '')
