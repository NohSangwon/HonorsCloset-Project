...
var BasketTable = function (data) {
    var saleProductTable = document.querySelector('#BasketTable');
	...
	img.addEventListener('click', openModal(data[i].Product_Code));
        ...
        var td = document.createElement('td');
        td.id = 'DelBtn';
        td.innerHTML = '<a id="DeleteBasket' + data[i].Product_Code + '" style="cursor:pointer; text-decoration:none; font-size:30px"><i class="icon-remove"></i></a>';
        tr.appendChild(td);

        tbody.appendChild(tr);

        document.querySelector('#DeleteBasket' + data[i].Product_Code).addEventListener('click', DeleteBasket(data[i].Product_Code));
...
var DeleteBasket = function (code) {
    var code_ = code;
    ...
    return function () {
        ...
        if (del) {
            $.ajax({
                type: "POST",
                url: '/ProductCLIENT/DeleteBasket',
                data: {
                    code: code
                },
                success: function (data) {
